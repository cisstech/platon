...
<Callout type="info">
Le contenu de l'opérateur `==` est interprété dans un nouveau contexte, donc la syntaxe d'affectation de variable `ple` n'est pas utilisable à l'intérieur de cet opérateur. Si le contenu de l'opérateur est un script à exécuter dans une sandbox, comme pour les clés `builder`, `grader`, ou `hint.next`, le script doit être écrit dans le langage de programmation de la sandbox choisie. Vous aurez accès aux variables déclarées dans le contexte de l'exercice, et toute variable globale déclarée dans cette clé sera ajoutée au contexte de l'exercice après l'exécution du script.
</Callout>
...
Pour lire le contenu des fichiers dans les scripts de la sandbox (builder, grader, hint.next), utilisez les fonctions de lecture de Python si la sandbox est en Python. Pour une sandbox Node, utilisez la fonction spéciale `readFile: (path: string, options?: ReadFileOptions)` pour lire le contenu d'un fichier. Le fichier sera accessible relativement à la racine de l'exercice `./`.
...
## Ordre des clés dans un ple

Bien que la plupart des déclarations de clés dans un fichier `.ple` puissent apparaître dans n'importe quel ordre, certaines règles importantes doivent être respectées :

1. **Instructions d'héritage et de composition** : Les directives `@extends` doivent apparaître en premier dans le fichier.

2. **Hiérarchie des clés** : Une clé parente doit être déclarée avant d'accéder à ses sous-clés.

```py
# Erreur - Tentative d'accès à une sous-clé avant la déclaration de sa clé parente
myobject.key1 = 0
myobject = {
  key1: 0
}
...
Pour une meilleure lisibilité et structure logique de l'exercice, voici un ordre recommandé pour les déclarations :

1. **Configuration de l'environnement**
   - `sandbox = "node" | "python"` (choix de l'environnement d'exécution)
   - `@include` (inclusion des fichiers nécessaires)

2. **Structure et dépendances**
   - Directives d'héritage et de composition

3. **Déclaration des éléments**
   - Composants de l'interface utilisateur
   - Variables globales utilisées par les scripts

4. **Scripts de fonctionnement**
   - `builder` (génération des données)
   - `grader` (évaluation des réponses)

5. **Contenu pédagogique**
   - `hint` (indices)
   - `theories` (ressources théoriques)
   - `solution` (solution de l'exercice)

6. **Interface utilisateur**
   - `title` (titre de l'exercice)
   - `statement` (énoncé)
   - `form` (formulaire de réponse)

Cette structure organise le code de manière à faciliter sa compréhension, en commençant par la configuration technique puis en progressant vers les éléments visibles par l'apprenant.
