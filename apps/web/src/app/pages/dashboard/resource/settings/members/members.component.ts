...
import { ResourceInvitationFormComponent, ResourceInvitationTableComponent, ResourceMemberTableComponent } from '@platon/feature/resource/browser';
...
export class ResourceMembersComponent implements OnInit, OnDestroy {
  ...
  protected async deleteInvitation(invitation: ResourceInvitation): Promise<void> {
    if (await this.presenter.deleteInvitation(invitation)) {
      this.invitations = this.invitations.filter(
        e => e.inviteeId !== invitation.inviteeId
      )
      this.changeDetectorRef.markForCheck();
    }
  }
