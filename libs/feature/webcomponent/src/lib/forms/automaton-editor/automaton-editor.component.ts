  ...
  implements OnInit, OnDestroy, WebComponentHooks<AutomatonEditorState>
{
  ...
  private get canvas() {
    return this.container.nativeElement.querySelector(
  ...
  private addListeners() {
    ...
    jsPlumb.on(
      ...
      'click',
      this.onClickContainer.bind(this)
    );
    jsPlumb.on(
      this.container.nativeElement,
      'dblclick',
      this.onDblClickContainer.bind(this)
    );
  ...
  private findConnection(transition: Transition) {
    return this.jsp
      ...
      .find(
        (e) =>
          e.sourceId === transition.fromState &&
          e.targetId === transition.toState
      ) as Connection;
  }
