...
import { DeleteAllNotificationsGQL, DeleteNotificationGQL, ListNotificationsGQL, MarkAllAsReadGQL, MarkAsReadGQL, NotifFragment, OnChangeNotificationsGQL } from '../models/notification.graphql.generated';
...
export class NotificationService {
  ...
  listUnreads() {
    return this.listNotificationsGQL.fetch({ filters: {
      limit: 10,
      unread: true
    } }).pipe(
      map(result => result.data.notifications.map(decodeFragment))
    );
  }
  ...
  markAsRead(id: string): Observable<Notification> {
    return this.markAsReadGQL.mutate({ id }).pipe(
      map(result => decodeFragment(result!.data!.markAsRead))
    );
  }

  markAllAsRead(): Observable<boolean> {
    return this.markAllAsReadGQL.mutate().pipe(
      map(result => result!.data!.markAllAsRead)
    );
  }

  deleteNotification(id: string): Observable<boolean> {
    return this.deleteNotificationGQL.mutate({ id }).pipe(
      map(result => result!.data!.deleteNotification)
    );
  }

  deleteAllNotifications(): Observable<boolean> {
    return this.deleteAllNotificationsGQL.mutate().pipe(
      map(result => result!.data!.deleteAllNotifications)
    );
  }
}
