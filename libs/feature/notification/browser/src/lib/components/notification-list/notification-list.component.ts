...
import { ChangeDetectionStrategy, ChangeDetectorRef, Component, EventEmitter, Inject, Injector, Input, OnChanges, Optional, Output } from '@angular/core';
...
export class NotificationListComponent implements OnChanges {
  ...
  ngOnChanges(): void {
    this.items = this.notifications.map((notification) => {
      return {
        renderer: this.parsers?.find(parser => parser.support(notification))
