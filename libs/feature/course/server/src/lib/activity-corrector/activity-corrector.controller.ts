...
export class ActivityCorrectorController {
  constructor(
    private readonly service: ActivityCorrectorService
  ) { }
  ...
  async create(
    ...
    return new ItemResponse({
      resource: Mapper.map(
        await this.service.findById(activityId, member.id), ActivityCorrectorDTO)
    });
  }
  ...
  async update(
    ...
    return new ItemResponse({
      resource: Mapper.map(
        await this.service.search(activityId), ActivityCorrectorDTO)
    });
  }
