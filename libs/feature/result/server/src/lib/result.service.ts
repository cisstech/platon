...
export class ResultService {
  ...
  async activityResults(activityId: string): Promise<ActivityResults> {
    ...
    return this.processData(
      await this.loadActivityData(activity)
    );
  }
  ...
  private processData(data: Data, waitForCorrections = false): ActivityResults {
    ...
    } = this.createExerciseResults(activityVariables, userResults)
    ...
    exerciseSessions.forEach(exerciseSession => {
      ...
      if (exerciseId) {
        ...
        const duration = exerciseSession.lastGradedAt && exerciseSession.startedAt
