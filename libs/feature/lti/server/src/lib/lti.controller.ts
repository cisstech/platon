...
export class LTIController {
  constructor(
    private readonly service: LTIService
  ) { }

   @Get('/lms')
   async searchLms(
     @Query() filters: LmsFiltersDTO
   ): Promise<ListResponse<LmsDTO>> {
  ...
  @Post('/lms')
  async createLms(
    @Body() input: CreateLmsDTO
  ): Promise<CreatedResponse<LmsDTO>> {
    const resource = Mapper.map(
      await this.service.createLms(input),
      LmsDTO
    );
   ...
   async updateLms(
     ...
     const resource = Mapper.map(
       await this.service.updateLms(id, input),
       LmsDTO
     );
     return new ItemResponse({ resource })
   }
