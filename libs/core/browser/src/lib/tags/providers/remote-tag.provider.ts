...
import { CreatedResponse, CreateLevel, CreateTopic, ItemResponse, Level, ListResponse, Topic, UpdateLevel, UpdateTopic } from "@platon/core/common";
...
export class RemoteTagProvider extends TagProvider {
  ...
  createTopic(input: CreateTopic): Observable<Topic> {
    return this.http.post<CreatedResponse<Topic>>(`/api/v1/topics`, input).pipe(
      map(response => response.resource)
    );
  }

  updateTopic(id: string, input: UpdateTopic): Observable<Topic> {
    return this.http.patch<ItemResponse<Topic>>(`/api/v1/topics/${id}`, input).pipe(
      map(response => response.resource)
    );
  }
  ...
  createLevel(input: CreateLevel): Observable<Level> {
    return this.http.post<CreatedResponse<Topic>>(`/api/v1/levels`, input).pipe(
      map(response => response.resource)
    );
  }

  updateLevel(id: string, input: UpdateLevel): Observable<Level> {
    return this.http.patch<ItemResponse<Level>>(`/api/v1/levels/${id}`, input).pipe(
      map(response => response.resource)
    );
  }
